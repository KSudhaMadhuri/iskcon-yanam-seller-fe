...
const Admin = () => {
  ...
  const deleteSeller = async (sellerId) => {
    ...
    const delConfirm = confirm("Seller will be deleted permanently, are you sure? ")
    if (delConfirm) {
      setDelSpin(sellerId)
      try {
        ...
        <div className="overflow-x-auto mt-6 pb-10">
          {
            sellers.length ?
              ...
              <table >
                ...
                <tbody>
                  {sellers.map((seller, index) => (
                    <tr key={seller._id}>
                      <td >{index + 1}</td>
                      <td className="text-nowrap">{seller.fullName.substring(0, 15)}</td>
                      <td>{seller.email}</td>
                      <td>
                        <div className="flex justify-center w-full">
                          <h5 className={seller.role === "admin" ? "flex items-center justify-center bg-green-600 h-8 w-[6rem] rounded font-semibold text-white" : ""}>
                            {seller.role}
                          </h5>
                        </div>
                      </td>
                      <td>
                        <div className="flex justify-center w-full">
                          ...
                          <h5 className={seller.admin === "admin" ? "flex items-center justify-center bg-indigo-600 text-white  h-8 w-[6rem] rounded font-semibold" : ""}>
                            {seller.admin}
                          </h5>
                        </div>
                      ...
                      <td >
                        <div className="flex gap-3 justify-center">
                          ...
                          <button onClick={() => updateModal(seller._id)} className=" mb-2 bg-blue-500 text-white w-24 h-9 rounded hover:bg-blue-900">
                            Update
                          </button>

                          {delSpin === seller._id ? <button
              ...
              <div className="text-xl mt-10 pt-10 text-center font-medium">Loading...</div>
          }
        </div>
      </div>
      ...
      {adminId ?
        <div className="seller-modal-card px-5">
          <div className="seller-sub-card bg-white rounded  p-3  w-[20rem]">
            ...
            <span  onClick={()=>setAdminId("")} className="cursor-pointer"><FaWindowClose size={20}/></span>
            </div>
            <div className="flex items-center  justify-between mt-3">
              ...
              <button  className=" mb-2 bg-blue-500 text-white w-24 h-9 rounded hover:bg-blue-900">Update</button>
            </div>
